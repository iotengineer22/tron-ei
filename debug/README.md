...
* **`debug/` 直下 (メイン評価プログラム)**:
  ...
  * `tron_pdm_detect_cpu.srec` (音声キーワード認識 CPU通常版)
    * NPUを用いずメインCPU（Cortex-M85）上で音声キーワード認識をすべて処理します（推論所要時間の比較・検証用）。

* **`base_firmware/` フォルダ配下 (検証・参考プログラム)**:
  以下の3つのプログラムは、基本ペリフェラルの単体動作確認や、参考用途の画像認識テスト用バイナリです。**必要に応じて個別に書き込みを行い、動作確認にご活用ください。**
  ...
  * `tron_edge_fomo_npu_type.srec` (FOMO基板部品検出 NPU高速版)
    * MIPIカメラの映像から電子基板上の極小の部品（Pico、Xiaoなど）をリアルタイムに同時識別し、個数と位置を検出・カウントする画像認識の移植参考バイナリです。
...
* カメラ画像AIテストの際は、同梱 of カメラモジュールをボード裏面のMIPI-CSI2コネクタへ接続してください。
...
### ⑤ FOMO基板部品検出 NPU高速版 (`base_firmware/tron_edge_fomo_npu_type.srec`)
* **動作・デモ**:
