...
* **CPU通常版との比較 (`base_firmware/tron_pdm_detect_cpu.srec`)**:
  * `base_firmware/tron_pdm_detect_cpu.srec`（CPU通常版）を書き込むと、推論時間が大幅に増加（数十ms〜数百ms）し、割り込みとスケジューリングにおけるCPU負荷の違いをシリアルログで比較確認できます。
