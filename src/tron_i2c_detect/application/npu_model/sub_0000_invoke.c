...
__attribute__((aligned(16))) uint8_t sub_0000_arena[224];
// Fast scratch arena not used for Ethos-U55
//  We will not create it for now and reuse the address of the other arena
// __attribute__((aligned(16))) static uint8_t sub_0000_fast_scratch[224];
...
int sub_0000_invoke(bool clean_outputs) {
  ...
  // Buffer sub_0000_model with size 3264 and address: 4294967295
  base_addrs[0] = (uint64_t)(uintptr_t)sub_0000_model_data;
  base_addrs_size[0] = sub_0000_model_data_size;
  // Buffer sub_0000_arena with size 224 and address: 0
  base_addrs[1] = (uint64_t)(uintptr_t) (sub_0000_arena+0);
  base_addrs_size[1] = 224;

  // Buffer sub_0000_fast_scratch with size 224 and address: 0
  base_addrs[2] = (uint64_t)(uintptr_t) (sub_0000_arena+0);
  base_addrs_size[2] = 224;

  // Buffer input_tensor_0 with size 39 and address: 0
  base_addrs[3] = (uint64_t)(uintptr_t) (sub_0000_arena+0);
  base_addrs_size[3] = 39;

  // Buffer output_tensor_0 with size 4 and address: 0
  if (clean_outputs) {
    memset(sub_0000_arena + 0, 0, 4);
  }
