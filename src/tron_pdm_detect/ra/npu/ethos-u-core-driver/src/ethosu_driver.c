...
int __attribute__((weak)) ethosu_semaphore_take(void *sem, uint64_t timeout)
{
    ...
    if (sem != ethosu_semaphore)
    {
        struct ethosu_driver *drv = registered_drivers;
        if (drv != NULL)
        {
            volatile struct NPU_REG *reg = drv->dev.reg;
            while (!reg->STATUS.cmd_end_reached &&
                   !reg->STATUS.bus_status &&
                   !reg->STATUS.cmd_parse_error &&
                   !reg->STATUS.wd_fault &&
                   !reg->STATUS.ecc_fault)
            {
                // Spin wait
            }
