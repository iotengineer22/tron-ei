...
#endif
{
    ...
    extern struct sbrk_log_entry g_sbrk_log[16];
    extern volatile int g_sbrk_log_idx;

    char * current_block_address = (char *) &g_heap[current_block_offset];

    if (g_sbrk_log_idx < 16) {
        g_sbrk_log[g_sbrk_log_idx].incr = incr;
        g_sbrk_log[g_sbrk_log_idx].offset_before = current_block_offset;
        g_sbrk_log[g_sbrk_log_idx].ret = (uint32_t)current_block_address;
        g_sbrk_log_idx++;
    }
