...
fsp_err_t RM_ETHOSU_Open (rm_ethosu_ctrl_t * p_api_ctrl, rm_ethosu_cfg_t const * const p_cfg)
{
    ...
    extern void tm_printf(const unsigned char *format, ...);
    tm_printf((const unsigned char *)"[RM_ETHOSU_Open] p_ctrl = 0x%08X\n", (unsigned int)p_ctrl);
    tm_printf((const unsigned char *)"[RM_ETHOSU_Open] p_ctrl->p_ext_cfg = 0x%08X\n", (unsigned int)p_ctrl->p_ext_cfg);
    if (p_ctrl->p_ext_cfg) {
        tm_printf((const unsigned char *)"[RM_ETHOSU_Open] p_ctrl->p_ext_cfg->p_dev = 0x%08X\n", (unsigned int)p_ctrl->p_ext_cfg->p_dev);
    }
